...
const BrandLogo = ({ className = '', light = false }: BrandLogoProps) => (
  <Link to="/" aria-label={marketing.header.home} className={`brand-logo ${light ? 'brand-logo-light' : ''} ${className}`}>
    <svg viewBox="0 0 32 32" aria-hidden="true" className="brand-mark">
      ...
      <rect className="brand-mark-accent" x="17.5" y="7" width="8" height="8" rx="2.5" />
    </svg>
    <span>Clipealo</span>
  </Link>
);
